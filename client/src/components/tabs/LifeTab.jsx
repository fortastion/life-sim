...
export default function LifeTab() {
  const { character, ageUp, pendingChoices } = useGameStore();
  ...
  const recentHistory = showFullHistory ? [...character.history].reverse() : [...character.history].reverse().slice(0, 10);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-4 pt-3 pb-2">
        <div className="bg-bg-card rounded-2xl p-3 border border-bg-border">
          <div className="flex items-center gap-3">
            {/* Avatar */}
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-4xl avatar-ring flex-shrink-0"
            ...
            <div className="text-center">
              ...
              <div className="text-sm font-bold text-blue-400">
                {character.career.employed || character.isRetired
                  ? formatMoney(Math.floor(character.career.salary * character.salaryMult), character.currency)
                  : '—'}
              </div>
            </div>
            <div className="text-center">
      ...
      <div className="px-4 mb-3">
        ...
        >
          {pendingChoices.length > 0 ? `⚠️ Resolve ${pendingChoices.length} Choice${pendingChoices.length > 1 ? 's' : ''} First` : `🎂 Age Up → ${character.age + 1}`}
        </motion.button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        ...
        {recentHistory.length === 0 && (
          <div className="text-center text-slate-500 py-8">No events yet. Age up to start your life!</div>
        )}
